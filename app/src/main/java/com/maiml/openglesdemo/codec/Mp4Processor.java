...
import com.maiml.openglesdemo.renderer.WrapRenderer;
...
public class Mp4Processor {
    ...
    private EGLHelper2 mEGLHelper;               //GL环境创建的帮助类
    ...
    private int mOutputVideoWidth=540;    //输出视频的宽度
    private int mOutputVideoHeight=960;   //输出视频的高度
    ...
    public void setRenderer(Renderer renderer){
        mRenderer=new WrapRenderer(renderer,mContext);
    }
    ...
    private boolean prepare() throws IOException {
        ...
        synchronized (PROCESS_LOCK){
            ...
            for (int i=0;i<count;i++){
                ...
                Log.d("tag","extractor format-->"+mExtractor.getTrackFormat(i));
...
//                mMuxer.setOrientationHint(videoRotation);
                ...
                if(mAudioDecoderTrack>=0){
                    ...
                    Log.d("tag","audio track-->"+format.toString());
                ...
                mDecodeThread=new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if(mVideoDecoderTrack>=0){
                            Log.d("tag","videoDecodeStep start");
                            while (mCodecFlag&&!videoDecodeStep());
                            Log.d("tag","videoDecodeStep end");
                            mGLThreadFlag=false;
                            try {
                                mSem.release();
                                mGLThread.join();
    ...
    private boolean videoDecodeStep(){
        ...
        while (true){
            ...
            if(mOutputIndex>=0){
                try {
                    Log.d("tag"," mDecodeSem.acquire ");
                    if(!isUserWantToStop){
                        mDecodeSem.acquire();
                    }
                    Log.d("tag"," mDecodeSem.acquire end ");
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
    ...
    private void glRunnable(){
        ...
        if(!ret)return;
        if(mRenderer==null){
            mRenderer=new WrapRenderer(null,mContext);
        }
        mRenderer.create();
        mRenderer.sizeChanged(mOutputVideoWidth,mOutputVideoHeight,mInputVideoWidth,mInputVideoHeight);
        while (mGLThreadFlag){
            try {
                Log.d("tag"," mSem.acquire ");
                mSem.acquire();
                Log.d("tag"," mSem.acquire end");
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            if(mGLThreadFlag){
        ...
        synchronized (PROCESS_LOCK){
            if(isStarted){
                if(mCodecFlag){
                    ...
                    if(mDecodeThread!=null&&mDecodeThread.isAlive()){
                        Log.d("tag","try to stop decode thread");
                        mDecodeThread.join();
                        Log.d("tag","decode thread stoped");
                    }
                    isUserWantToStop=false;
                }
            }
